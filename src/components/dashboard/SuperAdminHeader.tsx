...
const SuperAdminHeader: React.FC<SuperAdminHeaderProps> = ({ onSignOut }) => {
  ...
  const handleShowDebugInfo = async () => {
    ...
    setSessionInfo({
      ...
      expiresAt: data.session ? new Date(data.session.expires_at! * 1000).toLocaleString() : null,
            ...
            {sessionInfo && (
              <>
                <div className="p-2 bg-gray-50 rounded">
                  <p><strong>Current Route:</strong> {sessionInfo.currentRoute}</p>
                  <p><strong>Has Active Session:</strong> {sessionInfo.hasSession ? "Yes" : "No"}</p>
                  {sessionInfo.expiresAt && (
                    <p><strong>Session Expires:</strong> {sessionInfo.expiresAt}</p>
                  )}
                  {sessionInfo.error && (
                    <p className="text-red-600"><strong>Auth Error:</strong> {sessionInfo.error}</p>
                  )}
                </div>
                <div className="mt-4">
                  <Button 
