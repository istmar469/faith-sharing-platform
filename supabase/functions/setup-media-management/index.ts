...
serve(async (req) => {
  ...
  try {
    ...
    const { data: existingTables } = await supabaseClient
      ...
      .in('table_name', ['media_files', 'media_quotas'])

    if (existingTables && existingTables.length > 0) {
      return new Response(
        JSON.stringify({ message: 'Media management tables already exist' }),
        { 
    ...
    // Execute the media management migration
    const migrationSql = `
...
`
    ...
    if (error) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to create media management tables', details: error }),
        { 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 500 
        }
    ...
    const { error: bucketError } = await supabaseClient.storage.createBucket('media', {
      public: true,
      allowedMimeTypes: [
    ...
    return new Response(
      JSON.stringify({ 
        message: 'Media management system setup successfully',
        tablesCreated: ['media_files', 'media_quotas'],
        storageBucket: 'media'
      }),
      { 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to setup media management system', details: error.message }),
      { 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 500 
      }
